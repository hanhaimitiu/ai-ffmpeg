...
### 🤖 自然语言 Agent（LangGraph + 大模型驱动）

智能助手基于 **LangGraph 状态图**编排（plan → 校验 → 失败带提示重试 → execute），由**大模型**解析指令：支持任何 OpenAI 兼容接口（DeepSeek / OpenAI / 通义千问 / Moonshot / 智谱 GLM / **llama.cpp 本地模型** / Ollama 等）。Agent 会把"全部文件 + 完整流信息 + 操作能力 Schema"发给大模型，由它返回结构化操作计划再调用 ffmpeg 执行。

- **流级操作**：探测层输出全部流（多音轨/多字幕，含语言与轨道序号），可以「提取英语那条音轨」「换成第二条音轨」「音视频分流」（拆成纯视频 + 纯音频两个文件）
- **多文件协同**：不同文件做不同操作（「第一个文件提取音频，第二个转 720p」），或**合并多个视频**（分辨率/采样率不同也能拼，自动归一化重编码）
...
- 「提取英语那条音轨」「音视频分流」
- 「第一个文件提取音频，第二个转 480p」「把这几个视频合并成一个」
...
```bash
npm test             # 单元测试：LangGraph 链路(28) + 命令构建(28) + 会话存储(14)
npm run test:e2e     # 端到端冒烟：真实 ffmpeg 执行（双音轨分流/选轨/合并/多文件）
npm run test:real    # 真实本地大模型验证（含流操作/多文件/多轮用例，未启动时自动跳过）
```
...
│   └── agent/
│       ├── graph.js          # LangGraph 状态图：plan→重试→execute 编排
│       ├── agent.js          # 操作 Schema / 校验 / 多文件计划规范化 / 提示词
│       ├── executor.js       # 操作 → ffmpeg 命令（选轨/分流/合并/选项位置语义）
│       └── llm.js            # OpenAI 兼容接口调用（连通测试/模型列表）
