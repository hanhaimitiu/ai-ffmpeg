...
- 任务队列：串行执行、实时进度条、取消任务、历史记录
- 自动选择输出路径，绝不覆盖源文件

### 🤖 自然语言 Agent（双模式）
| 模式 | 说明 |
|------|------|
| **大模型模式（推荐）** | 配置任一 OpenAI 兼容接口（DeepSeek / OpenAI / 通义千问 / Moonshot / 智谱 GLM / Ollama 本地模型等），Agent 会把"媒体信息 + 操作能力 Schema"发给大模型，由它返回结构化操作再执行 |
| **本地规则模式** | 未配置 API Key 也能用。内置中英文规则解析器，覆盖 18 类常用操作，LLM 不可用或返回异常时自动回退到此模式 |

支持的操作（大模型模式下可自由组合，规则模式支持大部分）：
格式转换、裁剪片段、提取音频、静音、分辨率/码率/帧率调整、倍速/减速、倒放、旋转、音量、文字水印、压缩(CRF)、降噪、镜像翻转、封面/帧截图、转 GIF、查询媒体信息。

例：
- 「把 video.mp4 转成 mp3 并压缩」
- 「截取从 10 秒到 30 秒的片段，转 720p」
- 「在右下角加水印"我的视频"，然后加速 2 倍」
- 「这个视频有多长？」
...
配置成功后，智能助手会切换到大模型解析；未配置时自动使用本地规则解析。
...
应用会通过 `response_format: json_object` 让本地模型直接输出结构化操作 JSON（llama.cpp 与 Ollama 均支持），并在 LLM 异常时自动回退本地规则解析。
...
```bash
npm test             # 单元测试：解析器(33) + 命令构建(26) + LLM Agent 链路(9)
npm run test:e2e     # 端到端冒烟：真实 ffmpeg 执行（生成测试视频→解析→执行→校验产物）
npm run test:real    # 真实本地大模型验证（需 llama.cpp/Ollama 已启动，未启动时自动跳过）
```
